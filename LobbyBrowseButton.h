...
{
public:
	LobbyBrowseButton(GameClient* game_client, int x, int y, int width, int height, int text_size, std::string text1, std::string text2, std::string text3, std::string text4, std::string button_texture, std::string button_texture_highlighted, std::string button_texture_selected)
		: MenuButton(game_client, x, y, width, height, text_size, text1, button_texture, button_texture_highlighted)
	{
		...
		_button_text4 = _game_client->getResourceManager()->getTextureText(text4, { 0xFF, 0xFF, 0xFF }, text_size);
	}
	~LobbyBrowseButton();
